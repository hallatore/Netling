...
namespace Netling.Core
{
    public static class UrlJobExtensions
    {
        ...
        private static Task<UrlResult> GetResult(string url)
        {
            ...
            try
            {
                ...
                return Task.FromResult(new UrlResult((double)sw.ElapsedTicks / Stopwatch.Frequency * 1000, length, startTime, url, Thread.CurrentThread.ManagedThreadId));
            }
            catch (Exception)
            {
